...
public abstract class GenericAdapterListener<T> implements BaseAdapterListener<T> {
    ...
    public List<T> filterResults(List<T> masterList, String searchConstraint) {
        ...
        return Flowable.fromIterable(masterList)
                .filter(itemEncapsulator -> itemEncapsulator.toString().toLowerCase()
                        .contains(searchConstraint.trim().toLowerCase())).toList().blockingGet();
    }
}
